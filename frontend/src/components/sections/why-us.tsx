"use client";

import { Check } from "lucide-react";
import { BackgroundGrid } from "@/components/ui/background-grid";
import { Card } from "@/components/ui/card";
import { FadeInSection } from "@/components/ui/fade-in-section";
import { home as homeContent } from "@/content";

const whyUsContent = homeContent.whyUs;

export function WhyUsSection() {
  return (
    <section id="why-cyberlix" className="relative scroll-mt-24 overflow-hidden bg-background py-24 min-h-screen flex flex-col justify-center">
      <div className="absolute right-0 top-0 h-full w-1/2 bg-gradient-to-l from-primary/5 to-transparent" />
      <BackgroundGrid />

      <div className="layout-container relative z-10 grid gap-12 md:grid-cols-2 items-center">
        <FadeInSection className="space-y-6">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-primary">
            {whyUsContent.eyebrow}
          </p>
          <h2 className="text-3xl md:text-4xl font-bold leading-tight text-foreground">
            {whyUsContent.title} <span className="text-primary">{whyUsContent.highlight}</span>
          </h2>
          <p className="text-lg text-muted-foreground">
            {whyUsContent.description}
          </p>
        </FadeInSection>

        <FadeInSection delay={200}>
          <Card variant="glass" className="p-8">
            <ul className="space-y-4">
              {whyUsContent.reasons.map((reason) => (
                <li key={reason} className="flex items-start gap-4">
                  <div className="mt-1 flex-shrink-0 rounded-full bg-primary/20 p-1 text-primary">
                    <Check className="h-4 w-4" strokeWidth={3} />
                  </div>
                  <span className="text-base text-card-foreground">{reason}</span>
                </li>
              ))}
            </ul>
          </Card>
        </FadeInSection>
      </div>
    </section>
  );
}
